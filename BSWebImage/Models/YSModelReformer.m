...
+ (YSDataRecordModel *)dataRecordModelFromRunDatabaseDic:(NSDictionary *)runDic{
    
    YSDataRecordModel *heartRateModel = [YSDataRecordModel new];
    
    heartRateModel.startTime = [runDic[@"BeginTime"] integerValue];
    heartRateModel.endTime = [runDic[@"EndTime"] integerValue];
    heartRateModel.calorie = [runDic[@"Cost"] integerValue];
    heartRateModel.distance = [[NSString stringWithFormat:@"%0.2f",[runDic[@"AllDistance"]floatValue]] floatValue];
    
    YSHeartRateDataTransformModel *heartRateDataTransform = [[YSHeartRateDataTransformModel alloc] initWithDataString:runDic[@"HeartRate"]];
    heartRateModel.heartRateArray = heartRateDataTransform.dataArray;
    
    YSLocationDataTransformModel *locationDataTransformModel = [[YSLocationDataTransformModel alloc] initWithDataString:runDic[@"LocationPosition"]];
