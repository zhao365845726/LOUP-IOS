...
- (void)setupSettingsTypeArray
{
    ...
    if (KUserId && ![KUserId isEqualToString:@" "])//判断是否登录
    {
...
//                         [NSNumber numberWithInteger:YSSettingsTypeFeedback]];
            section1 = @[[NSNumber numberWithInteger:YSSettingsTypeFeedback],[NSNumber numberWithInteger:YSSettingsTypeMyVesion]];
...
{
    // 用户反馈
    YCFeedbackViewController *feedbackViewController = [[YCFeedbackViewController alloc]init];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if(cellType == YSSettingsTypeMyVesion){
        [self myVesion];
    }
...
- (void)myVesion
{
    YCMyVesionViewController* vesion = [[YCMyVesionViewController alloc]init];
    [self.navigationController pushViewController:vesion animated:YES];
}
