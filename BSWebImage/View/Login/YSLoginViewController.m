...
- (void)viewDidLoad {
    ...
    self.thirdPartLoginView.hidden = YES;
}
...
- (void)setupTextFields
{
    ...
    NSLog(@"textFieldHeight = %f",textFieldHeight);
...
//    return;
    ...
    [Mdic setValue:account forKey:@"Mobile"];
    [Mdic setValue:password forKey:@"Password"];
    WS(weakSelf);
    [ABHttpRequest ASyncPOST:KUserLoginUrl andParameters:Mdic andTimeOut:KTimeOut loading:YES success:^(BOOL success, int code, NSString *msg, id responseObject) {
        if(success){
            NSDictionary* infoDic = (NSDictionary*)responseObject;
            [[NSUserDefaults standardUserDefaults]setObject:infoDic[@"UserId"] forKey:@"userId"];
            //需要修改
            [[NSUserDefaults standardUserDefaults] setObject:infoDic[@"RealName"] forKey:@"RealName"];
            
              [self loginFinishWithUserInfoResponseDic:infoDic];
...
-(void)loginFinishWithUserInfoResponseDic:(NSDictionary *)Mdic{
    NSLog(@"登录成功后请求");
    [[YSLoadingHUD shareLoadingHUD] dismiss];
    [[[UIApplication sharedApplication] keyWindow] endEditing:YES];
    [self.delegate loginViewController:self loginFinishWithUserInfoResponseDic:Mdic];
