...
- (void)registerUserInfo
{
    ...
    [Mdic setValue:self.phoneNumber forKey:@"Tel"];
    [Mdic setValue:nickName forKey:@"Account"];
    [Mdic setValue:self.verificationCode forKey:@"TelVerCode"];
    [Mdic setValue:password forKey:@"Password"];
    
    [ABHttpRequest ASyncPOST:KUserRegisterUrl andParameters:Mdic andTimeOut:KTimeOut loading:YES success:^(BOOL success, int code, NSString *msg, id responseObject) {
        if(success){
            [[YSTipLabelHUD shareTipLabelHUD]showTipWithText:@"注册成功"];
