...
- (void)captchaButtonClicked:(UIButton *)button
{
    ...
    if (isValid)
    {
        ...
        [Mdic setValue:phoneNumber forKey:@"Tel"];
        
        [ABHttpRequest ASyncPOST:KSendSMSVerificationCode andParameters:Mdic andTimeOut:KTimeOut loading:NO success:^(BOOL success, int code, NSString *msg, id responseObject) {
            if(success){
                [weakSelf sendCaptchaSuccess];
...
- (void)registerUser
{
    ...
    YSRegisterUserInfoViewController *registerUserInfoViewController = [[YSRegisterUserInfoViewController alloc] init];
    registerUserInfoViewController.phoneNumber = phoneNumber;
    registerUserInfoViewController.verificationCode = captcha;
    [self.navigationController pushViewController:registerUserInfoViewController animated:YES];
