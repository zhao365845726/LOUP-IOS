    ...
    if (KUserId && ![KUserId isEqualToString:@" "]) {
        ...
        [ABHttpRequest ASyncPOST:KGetUserInfoUrl andParameters:dict andTimeOut:KTimeOut loading:NO success:^(BOOL success, int code, NSString *msg, id responseObject) {
            if(success){
                ...
                ;
                [self.userRecordView setUserName:[[NSUserDefaults standardUserDefaults]objectForKey:@"RealName"]
                                       headPhoto:[UIImage imageNamed:@"user_default_photo"]
                                   totalDistance:[[NSString stringWithFormat:@"%0.2f",[result[@"TotalDistance"]floatValue]/1000] floatValue]
                                   totalRunTimes:[result[@"TotalRunTimes"]integerValue]
                                       totalTime:[result[@"TotalUseTime"]integerValue]/60];
            }
        } failure:^(NSString *msg) {
            
        }];
    }else{
        [self.userRecordView setUserName:@"未登录"
            headPhoto:[UIImage imageNamed:@"user_default_photo"]
            totalDistance:0.0
            totalRunTimes:0
            totalTime:0];
