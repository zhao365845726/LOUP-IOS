...
- (void)commitRunData:(YSRunDatabaseModel *)runDataInfoModel{
    if(KUserId == nil || [KUserId isEqualToString:@" "]){
        return;
    }
    NSLog(@"KUserId = %@",KUserId);
    NSLog(@"locationDataString = %@",runDataInfoModel.locationDataString);
    if (runDataInfoModel.locationDataString == nil) {
        NSLog(@"位置数据为空");
    }
    NSLog(@"heartRateDataString = %@",runDataInfoModel.heartRateDataString);
    NSDictionary *parm = @ {
        @"uid" : KUserId,
        @"pace" : [NSNumber numberWithFloat:runDataInfoModel.pace],
        @"distance" : [NSNumber numberWithFloat:runDataInfoModel.distance],
        @"usetime" : [NSNumber numberWithInteger:runDataInfoModel.usetime],
        @"cost" : [NSNumber numberWithInteger:runDataInfoModel.cost],
        @"star" : [NSNumber numberWithInteger:runDataInfoModel.star],
        @"h_speed" : [NSNumber numberWithFloat:runDataInfoModel.hSpeed],
        @"l_speed" : [NSNumber numberWithFloat:runDataInfoModel.lSpeed],
        @"date" : runDataInfoModel.date,
        @"bdate" : [NSNumber numberWithInteger:runDataInfoModel.bdate],
        @"edate" : [NSNumber numberWithInteger:runDataInfoModel.edate],
        @"speed" : [NSNumber numberWithFloat:runDataInfoModel.speed],
        @"location" : runDataInfoModel.locationDataString == nil ? @" ": runDataInfoModel.locationDataString,
        @"heartdata" : runDataInfoModel.heartRateDataString == nil ? @" " : runDataInfoModel.heartRateDataString
    };
    NSLog(@"parm = %@",parm);
    NSString *url = @"http://lu.api.ymstudio.xyz//api/running/commitrundata";
...
- (void)recordRunData
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^(){
        ...
        //上传数据
        [self  commitRunData:runDatabaseModel];
